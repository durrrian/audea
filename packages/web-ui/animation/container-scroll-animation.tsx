...
export const ContainerScroll = ({
  ...
  return (
    <div className='h-[60rem] md:h-[80rem] flex items-center justify-center relative p-2 md:p-20' ref={containerRef}>
      ...
      >
        <HeaderScrollAnimation translate={translate} titleComponent={titleComponent} />
        <CardScrollAnimation rotate={rotate} translate={translate} scale={scale} users={users} />
      </div>
    </div>
  )
}

export const HeaderScrollAnimation = ({ translate, titleComponent }: any) => {
  return (
    <motion.div
      style={{
      ...
      <div className='bg-gray-100 h-full w-full rounded-2xl grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 overflow-hidden p-4'>
        {users.map((user, idx: number) => (
          ...
          >
            ...
            <img src={user.image} className='rounded-tr-md rounded-tl-md text-sm ' alt='thumbnail' />
            <div className='p-4'>
