...
interface Top3LoserTodayProps {
  allTransactions: StockTransaction[]
  allEmiten: Emiten[]
}

export function Top3LoserToday({ allTransactions, allEmiten }: Top3LoserTodayProps) {
  ...
  return (
    <TopGainerLoser
      ...
      data={data.items.sort((a, b) => a['Ptsl (+/-) %'] - b['Ptsl (+/-) %']).slice(0, 3)}
    />
  )
}
